...
export class DashboardComponent implements OnInit {
  ...
  ngAfterContentInit() {
    if (localStorage.getItem('experimentos')) {
      this.ListaExperimentos = JSON.parse(
        localStorage.getItem('experimentos')?.toString() + ''
      );
    ...
    return new Promise((resolve) => {
      ...
      if (da != null) {
        ...
        this.usuario = JSON.parse(
          localStorage.getItem('datos')?.toString() + ''
        );
  ...
  mostraHoja(id: any) {
    ...
    this.objHojaSeleccionada = new HojaMedicion();
    const componeteAnterior = document.getElementById(
      this.experimentoSeleccionado
    );
    const componentCollapAnterior = document.getElementById(
      this.experimentoSeleccionado + 'collapse'
    );
    ...
    const componeteActual = document.getElementById(
      this.experimentoSeleccionado
    );
    const componentCollapActual = document.getElementById(
      this.experimentoSeleccionado + 'collapse'
    );
  ...
  mostraRepeticiones(id: any) {
    ...
    const componentCollapAnterior = document.getElementById(
      this.hojaSeleccionada + 'collapse'
    );
      ...
      if (hoja.id == this.hojaSeleccionada) {
        ...
        registros?.forEach((r) => {
          if (r.parcela?.includes('R1')) {
            this.repeticion1.push({
              parecela: this.ListaPercelas[i],
              registro: r,
            });
          }
          if (r.parcela?.includes('R2')) {
            this.repeticion2.push({
              parecela: this.ListaPercelas[i],
              registro: r,
            });
          }
          if (r.parcela?.includes('R3')) {
            this.repeticion3.push({
              parecela: this.ListaPercelas[i],
              registro: r,
            });
          }
          if (r.parcela?.includes('R4')) {
            this.repeticion4.push({
              parecela: this.ListaPercelas[i],
              registro: r,
            });
          }
  ...
  asicnarTipo(id: string) {
    ...
    if (document.getElementById('R5')?.classList.contains('active')) {
      this.asignarRepeticion('R5');
      this.repeticionSeleccionada = '5';
    }
  }
  asignarRepeticion(id: string) {
    ...
    switch (id) {
      ...
      case 'R5':
        ...
        this.generargraficaAll(this.repeticion1,this.repeticion2,this.repeticion3,this.repeticion4,0);
        this.repeticionSeleccionada = '5';
        break;
      default:
        break;
  ...
  recuperarNombreTestigo(testigo: string | undefined) {
    for (
      var i = 0;
      i < this.objExperimentoSeleccionado.listaVariedades.length;
      i++
    ) {
  ...
  generargrafica(repeticion: Columna[], num: number) {
    ...
    for (var i = 0; i < repeticion.length; i++) {
      if (this.altura) {
        ...
        var promedio =
          (repeticion[i].registro.listaSurcos[0].cantidad_tallos +
            repeticion[i].registro.listaSurcos[1].cantidad_tallos) /
          2;
        datos.push(Number(promedio.toFixed()));
      }
      etiquetas.push(
        this.ListaVariedad[repeticion[i].parecela.numero - 1].tratamiento + ''
      );
      /*
   if(strColor){
    strColor="#"+strColor.substring(3,strColor.length-1)+"0"
   ...
   */
      var strColor =
        this.ListaVariedad[repeticion[i].parecela.numero - 1].color + '';
      color.push(strColor);
    }
    ...
    console.log(color);
    this.barCharDatos.datasets.push({
      data: datos,
  ...
  generargraficaAll(
    ...
    var etiquetas: string[] = [];
    var color: string[] = [];
    for (var i = 0; i < repeticion1.length; i++) {
      if (this.altura) {
        datos.push(Number(repeticion1[i].registro.promedio.toFixed(2)));
      } else {
        var promedio =
          (repeticion1[i].registro.listaSurcos[0].cantidad_tallos +
            repeticion1[i].registro.listaSurcos[1].cantidad_tallos) /
          2;
        datos.push(Number(promedio.toFixed()));
      }
      etiquetas.push(
        this.ListaVariedad[repeticion1[i].parecela.numero - 1].tratamiento + ''
      );
      var strColor =
        this.ListaVariedad[repeticion1[i].parecela.numero - 1].color + '';
      color.push(strColor);
    }
    for (var i = 0; i < repeticion2.length; i++) {
      if (this.altura) {
        datos[i] =
          datos[i] + Number(repeticion2[i].registro.promedio.toFixed(2));
      } else {
        var promedio =
          (repeticion2[i].registro.listaSurcos[0].cantidad_tallos +
            repeticion2[i].registro.listaSurcos[1].cantidad_tallos) /
          2;
        datos[i] = datos[i] + Number(promedio.toFixed());
      }

    }
    for (var i = 0; i < repeticion3.length; i++) {
      if (this.altura) {
        datos[i] =
          datos[i] + Number(repeticion3[i].registro.promedio.toFixed(2));
      } else {
        var promedio =
          (repeticion3[i].registro.listaSurcos[0].cantidad_tallos +
            repeticion3[i].registro.listaSurcos[1].cantidad_tallos) /
          2;
        datos[i] = datos[i] + Number(promedio.toFixed());
      }

    }
    for (var i = 0; i < repeticion4.length; i++) {
      if (this.altura) {
        datos[i] =
          datos[i] + Number(repeticion4[i].registro.promedio.toFixed(2));
      } else {
        var promedio =
          (repeticion4[i].registro.listaSurcos[0].cantidad_tallos +
            repeticion4[i].registro.listaSurcos[1].cantidad_tallos) /
          2;
        datos[i] = datos[i] + Number(promedio.toFixed());
      }
    }
    for(var i = 0; i < datos.length; i++){
        datos[i]=Number((datos[i]/4).toFixed(2))
    }
  ...
  downloadPDF() {
    ...
    if (DATA != null) {
      html2canvas(DATA, options)
        ...
        .then((docResult) => {
          if (
            this.objExperimentoSeleccionado.nombre != null &&
            this.objHojaSeleccionada.mesCrecimiento != null
          ) {
            docResult.save(
              this.objExperimentoSeleccionado.nombre +
                ' Mes' +
                this.objHojaSeleccionada.mesCrecimiento +
                'R' +
                this.repeticionSeleccionada
            );
  ...
  canDeactivate(): Observable<boolean> | Promise<boolean> | boolean {
    // Add your logic here to determine if the user can deactivate or leave the page
    // For example, you can show a confirmation dialog and return its result as an Observable

    // Sample code to show a confirmation dialog using the browser's built-in confirm dialog
